...
export function PageDnsRecords() {
  ...
  const handleclickapi = async () => {
    const apiKey = router.query.apiToken;
    const zoneId = router.query.zoneId;
  
    try {
      const response = await fetch('/api/dns/route', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setState((s) => ({ ...s, message: 'DNS records created successfully.' }));
      } else {
        setState((s) => ({ ...s, message: `Error: ${data.message}` }));
      }
    } catch (error) {
      console.error('Error creating DNS records:');
        ...
        <Form>
          <fieldset disabled={state.isInProgress}>
            <div className="bg-white py-4 sticky-top">
              ...
              <button className="btn btn-primary mt-7 " onClick={handleclickapi} type="button">
           Add Bulk Record
          </button>
  ...
  }, [listDnsRecords, router.isReady,handleclickapi]);
